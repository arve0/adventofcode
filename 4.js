...
function parseRoom (str) {
  var sectorPos = str.search(/[0-9]{3}/);
  return {
    name: str.slice(0, sectorPos - 1),
    sector: parseInt(str.slice(sectorPos, sectorPos + 3)),
    checksum: str.slice(sectorPos + 4, -1)
  }
}

assert.deepEqual(parseRoom('aaaaa-bbb-z-y-x-123[abxyz]'), {
...
function checksum (str) {
  ...
  return Array.from(uniqKeys).sort((a, b) => {
...
var rooms = getInput().split('\n').map(parseRoom);
var realRooms = rooms.filter(isRealRoom);
var sectorIdSum = realRooms.reduce((sum, room) => sum + room.sector, 0);

console.log(`Sum of sector id's of real rooms: ${sectorIdSum}`);
...
function decipherRoomName (room) {
  room.realName = '';
  for (var i = 0; i < room.name.length; i += 1) {
    var letter = room.name[i];
    if (letter === '-') {
      room.realName += ' ';
      continue;
    }
    var j = (alphabet.indexOf(letter) + room.sector) % alphabet.length;
    room.realName += alphabet[j];
  }
}

realRooms.forEach(decipherRoomName);

function findNorthPoleObjects (room) {
  if (room.realName.indexOf('north') !== -1 &&
      room.realName.indexOf('pole') !== -1 &&
      room.realName.indexOf('object') !== -1) {
    console.log('Found room:');
    console.dir(room);
  }
}

realRooms.forEach(findNorthPoleObjects);

function getInput () {
  return `gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]
