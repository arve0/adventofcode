function parse (str) {
  var r = /(turn on|turn off|toggle) ([0-9]+),([0-9]+) through ([0-9]+),([0-9]+)/;
  var m = str.match(r);
  if (!m || m.length !== 6) {
    throw new Error(`Warning: Unable to parse "${str}"`);
  }
  var start = [ parseInt(m[2]), parseInt(m[3]) ];
  var stop = [ parseInt(m[4]), parseInt(m[5]) ];
  return {
    str: m[0],
    cmd: m[1],
    start: start,
...
var actions = {
  ...
  'toggle': function (cmd) {
    ...
    ctx.putImageData(img, cmd.start[0], cmd.start[1]);
  }
};
...
cmds.forEach(cmd => actions[cmd.cmd](cmd));
