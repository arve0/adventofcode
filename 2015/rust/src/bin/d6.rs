...
fn main () {
    ...
    for cmd in get_input().split("\n").flat_map(Cmd::from_str) {
        match cmd.kind {
            ...
            CmdKind::TOGGLE => map_mut_2d(&mut lights, cmd.xrange, cmd.yrange, |x| !x),
        }
    }
    ...
    for cmd in get_input().split("\n").flat_map(Cmd::from_str) {
        match cmd.kind {
            ...
            CmdKind::TOGGLE => map_mut_2d(&mut lights, cmd.xrange, cmd.yrange, |x| x + 2),
        }
    }
...
fn parse_kind(text: &str) -> CmdKind {
    if text.starts_with("turn on") {
        CmdKind::ON
    } else if text.starts_with("turn off") {
        CmdKind::OFF
    } else {
        CmdKind::TOGGLE
    }
...
fn count(lights: &Vec<Vec<bool>>) -> u32 {
    let mut count: u32 = 0;
    for row in lights {
        for el in row {
            if *el {
...
fn count_2(lights: &Vec<Vec<u32>>) -> u32 {
    let mut count: u32 = 0;
    for row in lights {
        for el in row {
            count += *el;
