...
assert.deepEqual(strToAddress('abba[mnop]qrst'), {
    addresses: ['abba', 'qrst'],
    hypernets: ['mnop']
});
...
function addressSupportsTLS (address) {
    var f = address.addresses.map(isABBA).reduce((t, i) => t || i ? true : false, false);
    var h = address.hypernets.map(isABBA).reduce((t, i) => t || i ? true : false, false);
    return f && !h;
}

assert(addressSupportsTLS(strToAddress('abba[mnop]qrst')));
assert(!addressSupportsTLS(strToAddress('abcd[bddb]xyyx')));
assert(!addressSupportsTLS(strToAddress('aaaa[qwer]tyui')));
assert(addressSupportsTLS(strToAddress('ioxxoj[asdfgh]zxcvbn')));
assert(addressSupportsTLS(strToAddress('asdf[ffff]asdf[asdf]ioxxoj[asdfgh]zxcvbn')));

var addresses = getInput().split('\n').map(strToAddress);
var numberOfTLS = addresses.map(addressSupportsTLS).reduce((sum, abba) => abba ? sum + 1 : sum, 0);

console.log(numberOfTLS);
...
function ABA (str) {
    ...
    for (var i = 0; i < str.length - 2; i += 1) {
        if (str[i] === str[i + 2] &&
            str[i] !== str[i + 1]) {
                aba.push({ A: str[i], B: str[i + 1] });
        }
    }
    return (aba.length !== 0) ? aba : false;
}

function isBAB (str, ab) {
    for (var i = 0; i < str.length - 2; i += 1) {
        if (str[i] === str[i + 2] &&
            str[i] === ab.B &&
            str[i + 1] === ab.A) {
...
function addressSupportsSSL (address) {
    var abaArrays = address.addresses.map(ABA).filter(ab => ab);
    var aba = [].concat.apply([], abaArrays);  // flatten
    if (aba.length === 0) {
        return false;
    }
    var bab = address.hypernets.map((h, i) => {
        for (var i = 0; i < aba.length; i += 1) {
            if (isBAB(h, aba[i])) {
                return true;
            }
        } 
...
debugger;
assert(addressSupportsSSL(strToAddress('aba[bab]xyz')));
assert(!addressSupportsSSL(strToAddress('xyx[xyx]xyx')));
assert(addressSupportsSSL(strToAddress('aaa[kek]eke')));
assert(addressSupportsSSL(strToAddress('zazbz[bzb]cdb')));

var numberOfSSL = addresses.map(addressSupportsSSL).filter(i => i).length;

console.log(numberOfSSL);


function getInput () {
    return `xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
