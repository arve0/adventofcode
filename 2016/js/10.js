...
Array.prototype.pick = function (key, val) {
  return this.find(i => i[key] === val);
};
Array.prototype.popMax = function () {
  var max = this.reduce((max, i) => i > max ? i : max, this[0]);
  this.splice(this.indexOf(max), 1);
  return max;
};
Array.prototype.popMin = function () {
  var min = this.reduce((min, i) => i < min ? i : min, this[0]);
...
function parse (str) {
  ...
  return lines.map(line => {
    if (line.indexOf('bot') === 0) {
      var [m, num, lowType, lowNum, highType, highNum] = line.match(/([0-9]+) gives low to ([a-z]+) ([0-9]+) and high to ([a-z]+) ([0-9]+)/);
      return { bot: true,
        num: parseInt(num),
        lowType, lowNum: parseInt(lowNum),
        highType, highNum: parseInt(highNum),
        values: [] };
    } else if (line.indexOf('value') === 0) {
      var [m, value, to] = line.match(/(\d+) goes to bot (\d+)/);
      return { value: parseInt(value), to: parseInt(to) };
    }
  });
}

var input = parse(getInput());
var bots = input.filter(i => i.bot);
var values = input.filter(i => i.value);
var outputs = input.reduce((arr, i) => {
  if (i.lowType === 'output') {
    arr[i.lowNum] = [];
  }
  if (i.highType === 'output') {
    arr[i.highNum] = [];
  }
  return arr;
}, []);
...
function whoIsSorting (bots, a, b, outputs) {
  while (bots.reduce((remaining, b) => remaining + b.values.length, 0) > 0) {
    for (var bot of bots) {
      ...
      debugger;
      assert.equal(bot.values.length, 2);
      var max = bot.values.popMax();
      var min = bot.values.popMin();
      if ((max === a && min === b) ||
          (min === a && max === b)) {
      ...
      if (bot.lowType === 'output') {
        ...
        var low = bots.pick('num', bot.lowNum);
        low.values.push(min);
        assert(low.values.length <= 2);
      }
      if (bot.highType === 'output') {
        outputs[bot.highNum].push(max);
      } else {
        var high = bots.pick('num', bot.highNum);
        high.values.push(max);
        assert(high.values.length <= 2);
      }
      debugger;
    }
  }
...
whoIsSorting(bots, 61, 17, outputs);
console.log(outputs);

function getInput () {
  return `bot 127 gives low to output 1 and high to bot 180
